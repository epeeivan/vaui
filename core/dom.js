...
class uis {
    ...
    append(struct) {
        this.sel.append(this.createElementFromStructure(struct));
    }
    //
    before(struct) {
    ...
    createElement(obj) {
        ...
        if (Object.hasOwnProperty.call(obj, "name") || obj.name != undefined) {
            ...
            if (
                ...
                //
                for (const key1 in obj.events) {
                    for (const key2 in obj.events[key1]) {
                        element.addEventListener(key1, function (e) {
                            // console.log(key2)
                            window[key2](element, obj.events[key1][key2], e);
                        });
                    }
                }
            }
